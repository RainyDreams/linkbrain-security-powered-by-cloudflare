...
// .wrangler/tmp/bundle-bPJz1y/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-bPJz1y/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
var verifyPassword = /* @__PURE__ */ __name(async (password, env2) => {
  ...
  console.log(env2.ADMIN_PASSWORD_SALT, env2);
  ...
  let diff = 0;
  const hash = env2.ADMIN_PASSWORD_HASH;
...
var errorResponse = /* @__PURE__ */ __name((msg, status = 400, code = 4e3, stack) => jsonResponse(null, status, code, stack || msg), "errorResponse");
var Money = {
  toCent: /* @__PURE__ */ __name((yuan) => Math.round((parseFloat(yuan) + Number.EPSILON) * 100), "toCent"),
  toYuan: /* @__PURE__ */ __name((cent) => parseFloat((cent / 100).toFixed(2)), "toYuan"),
  calcCommission: /* @__PURE__ */ __name((amountCent) => Math.max(500, Math.round(amountCent * 25e-5)), "calcCommission"),
  // 万2.5, 最低5元
  calcTax: /* @__PURE__ */ __name((amountCent) => Math.round(amountCent * 5e-4), "calcTax")
  // 万5
};
var Time = {
  // 获取东八区当前时间对象
  getCST: /* @__PURE__ */ __name(() => {
    const now = /* @__PURE__ */ new Date();
    const utc = now.getTime() + now.getTimezoneOffset() * 6e4;
    return new Date(utc + 36e5 * 8);
  }, "getCST"),
  // 格式化输出
  formatTime: /* @__PURE__ */ __name((date) => {
    return date.toISOString().replace("T", " ").substring(0, 19);
  }, "formatTime"),
  // 严格判断是否为交易时间
  isMarketOpen: /* @__PURE__ */ __name(() => {
...
var placeOrder = /* @__PURE__ */ __name(async (env2, body) => {
  ...
  if (side === "BUY") {
    ...
    const res = await env2.DB.batch([
      env2.DB.prepare("UPDATE account SET frozen_balance = frozen_balance + ? WHERE id=1 AND (balance - frozen_balance) >= ?").bind(freeze, freeze),
      env2.DB.prepare("INSERT INTO orders (symbol, name, side, price, qty, freeze_amount, status) VALUES (?, ?, 'BUY', ?, ?, ?, 'PENDING')").bind(marketData.symbol, marketData.name, priceCent, qty, freeze)
    ]);
    if (res[0].meta.changes === 0) return errorResponse("\u53EF\u7528\u8D44\u91D1\u4E0D\u8DB3", 400, 4001);
  } else {
    const res = await env2.DB.batch([
      env2.DB.prepare("UPDATE holdings SET available_qty = available_qty - ? WHERE symbol=? AND available_qty >= ?").bind(qty, symbol, qty),
      env2.DB.prepare("INSERT INTO orders (symbol, name, side, price, qty, status) VALUES (?, ?, 'SELL', ?, ?, 'PENDING')").bind(marketData.symbol, marketData.name, priceCent, qty)
    ]);
...
var matchOrders = /* @__PURE__ */ __name(async (env2) => {
  ...
  for (const o of orders) {
    ...
    if (isMatch) {
      await executeTrade(env2, o, curr);
    }
  }
}, "matchOrders");
async function executeTrade(env2, order, exePrice) {
  const accBefore = await env2.DB.prepare("SELECT balance, frozen_balance FROM account WHERE id=1").first();
  const snap = await env2.DB.prepare("SELECT total_assets FROM snapshots ORDER BY date DESC LIMIT 1").first();
  const totalAssets = snap ? snap.total_assets : accBefore.balance + accBefore.frozen_balance;
  const holdRes = await env2.DB.prepare("SELECT quantity FROM holdings WHERE symbol=?").bind(order.symbol).first();
  const oldQty = holdRes ? holdRes.quantity : 0;
  const oldVal = oldQty * exePrice;
  const prePosRatio = (oldVal / totalAssets * 100).toFixed(2);
  ...
  const settleAmt = order.side === "BUY" ? tradeAmt + fee : tradeAmt - fee - tax;
  const newQty = order.side === "BUY" ? oldQty + order.qty : oldQty - order.qty;
  const newVal = newQty * exePrice;
  const postPosRatio = (newVal / totalAssets * 100).toFixed(2);
  const stmts = [
    env2.DB.prepare("UPDATE orders SET status='FILLED', filled_qty=?, updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(order.qty, order.id)
  ];
  if (order.side === "BUY") {
    stmts.push(env2.DB.prepare("UPDATE account SET balance = balance + (? - ?), frozen_balance = frozen_balance - ? WHERE id=1").bind(order.freeze_amount, settleAmt, order.freeze_amount));
    const oldH = await env2.DB.prepare("SELECT total_cost, quantity FROM holdings WHERE symbol=?").bind(order.symbol).first();
    if (oldH) {
      const nc = oldH.total_cost + settleAmt;
      const nq = oldH.quantity + order.qty;
      stmts.push(env2.DB.prepare("UPDATE holdings SET quantity=?, total_cost=?, avg_cost=?, updated_at=CURRENT_TIMESTAMP WHERE symbol=?").bind(nq, nc, Math.round(nc / nq), order.symbol));
    } else {
      stmts.push(env2.DB.prepare("INSERT INTO holdings (symbol, name, quantity, available_qty, avg_cost, total_cost) VALUES (?, ?, ?, 0, ?, ?)").bind(order.symbol, order.name, order.qty, Math.round(settleAmt / order.qty), settleAmt));
    }
  } else {
    const oldH = await env2.DB.prepare("SELECT total_cost, quantity, avg_cost FROM holdings WHERE symbol=?").bind(order.symbol).first();
    if (oldH && oldH.quantity > order.qty) {
      const newQty2 = oldH.quantity - order.qty;
      const newTotalCost = Math.round(oldH.avg_cost * newQty2);
      stmts.push(env2.DB.prepare("UPDATE account SET balance = balance + ? WHERE id = 1").bind(settleAmt));
      stmts.push(env2.DB.prepare("UPDATE holdings SET quantity = ?, total_cost = ?, updated_at = CURRENT_TIMESTAMP WHERE symbol = ?").bind(newQty2, newTotalCost, order.symbol));
    } else {
      stmts.push(env2.DB.prepare("UPDATE account SET balance = balance + ? WHERE id = 1").bind(settleAmt));
      stmts.push(env2.DB.prepare("DELETE FROM holdings WHERE symbol = ?").bind(order.symbol));
    }
  }
  ...
  async fetch(request, env2, ctx) {
    ...
    try {
      if (url.pathname === "/api/public/overview" && method === "GET") {
        ...
        const { results: holdings } = await env2.DB.prepare("SELECT * FROM holdings WHERE quantity > 0").all();
        const { results: logs } = await env2.DB.prepare("SELECT * FROM trades ORDER BY id DESC LIMIT 10").all();
        ...
        const totalAssetsCent = account.balance + account.frozen_balance + marketCap;
        ...
        const lastSnap = snaps.length > 0 ? snaps[snaps.length - 1] : { total_assets: account.initial_capital };
        const dayPnlCent = totalAssetsCent - lastSnap.total_assets;
        const dayReturnRate = (dayPnlCent / lastSnap.total_assets * 100).toFixed(2);
        let maxDrawdown = 0;
        let peak = 0;
        [...snaps, { total_assets: totalAssetsCent }].forEach((s) => {
          if (s.total_assets > peak) peak = s.total_assets;
          const dd = (peak - s.total_assets) / peak;
        ...
        return jsonResponse({
          ...
          // 按仓位排序
          // 调仓日志格式：酒etf 512690 0.00% -> 10.23% 成本0.543 现价0.634
          logs: logs.map((l) => ({
            text: `${l.name} ${l.symbol} ${l.pre_pos_ratio}% -> ${l.post_pos_ratio}%`,
            detail: `\u6210\u672C${Money.toYuan(l.price)} | ${l.trade_time.split(" ")[1].substring(0, 5)}`,
            side: l.side,
            time: l.trade_time
          })),
          charts: {
            ...
            // 加上今天的实时点位
            latest: { date: (/* @__PURE__ */ new Date()).toISOString().split("T")[0], value: totalAssetsYuan }
          }
        });
      }
      if (url.pathname === "/api/public/comments") {
        if (method === "GET") {
          const { results } = await env2.DB.prepare("SELECT * FROM comments ORDER BY id DESC LIMIT 50").all();
          return jsonResponse(results);
        }
        if (method === "POST") {
          const body = await request.json();
          await env2.DB.prepare("INSERT INTO comments (nickname, content) VALUES (?, ?)").bind(body.nickname || "Guest", body.content).run();
          return jsonResponse({ msg: "ok" });
        }
      }
      if (url.pathname === "/api/auth/login" && method === "POST") {
        const { password } = await request.json();
        if (await verifyPassword(password, env2)) {
          return jsonResponse({ token: signToken(env2) });
        }
        return errorResponse("Password Error", 401);
      }
      if (url.pathname.startsWith("/api/admin/")) {
        if (!verifyAuth(request, env2)) return errorResponse("Unauthorized", 401);
        if (url.pathname === "/api/admin/dashboard") {
          const acc = await env2.DB.prepare("SELECT * FROM account WHERE id=1").first();
          const { results: holds } = await env2.DB.prepare("SELECT * FROM holdings").all();
          let mktCap = 0;
          for (const h of holds) {
            const m = await fetchStockPrice(h.symbol);
            if (m) mktCap += Money.toCent(m.price) * h.quantity;
          }
          return jsonResponse({
            balance: Money.toYuan(acc.balance),
            available: Money.toYuan(acc.balance - acc.frozen_balance),
            market_cap: Money.toYuan(mktCap),
            total: Money.toYuan(acc.balance + mktCap)
          });
        }
        ...
        if (url.pathname === "/api/admin/trade") {
          const res = await placeOrder(env2, await request.json());
          ctx.waitUntil(matchOrders(env2));
          return res;
        }
        if (url.pathname === "/api/admin/cancel") {
          const { order_id } = await request.json();
          return await cancelOrder(env2, order_id);
        }
        if (url.pathname === "/api/admin/holdings") {
          ...
          for (const h of results) {
            ...
            list.push({
              ...h,
              // 关键：这里必须转为“元”，且 avg_cost 本身在数据库是“分”，也要转
              current_price: m ? parseFloat(m.price) : Money.toYuan(h.avg_cost),
        ...
        if (url.pathname === "/api/admin/transfer" && method === "POST") {
          ...
          if (type === "IN") {
            await env2.DB.prepare("UPDATE account SET balance = balance + ? WHERE id = 1").bind(amountCent).run();
            return jsonResponse({ message: `\u5165\u91D1\u6210\u529F: \xA5${amount}` });
          } else {
            const acc = await env2.DB.prepare("SELECT (balance - frozen_balance) as available FROM account WHERE id = 1").first();
            if (acc.available < amountCent) return errorResponse("\u53EF\u7528\u4F59\u989D\u4E0D\u8DB3", 400, 4001);
            await env2.DB.prepare("UPDATE account SET balance = balance - ? WHERE id = 1").bind(amountCent).run();
            return jsonResponse({ message: `\u51FA\u91D1\u6210\u529F: \xA5${amount}` });
          }
        }
      }
      return errorResponse("Not Found", 404);
    } catch (e) {
      return errorResponse(e.message, 500, 5e3, e.stack);
    }
  },
