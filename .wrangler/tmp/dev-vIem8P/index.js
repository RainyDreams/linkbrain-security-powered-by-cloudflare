...
// .wrangler/tmp/bundle-LAGc5y/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-LAGc5y/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-LAGc5y/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
var verifyPassword = /* @__PURE__ */ __name(async (password, env2) => {
  const encoder = new TextEncoder();
  const salt = encoder.encode(env2.ADMIN_PASSWORD_SALT || "secure_default_salt");
  ...
  let diff = 0;
  const hash2 = env2.ADMIN_PASSWORD_HASH;
  for (let i = 0; i < derivedHex.length; i++) {
    diff |= derivedHex.charCodeAt(i) ^ hash2.charCodeAt(i);
  }
  return diff === 0 && derivedHex.length === hash2.length;
}, "verifyPassword");
var signToken = /* @__PURE__ */ __name((env2) => {
  return import_jsonwebtoken.default.sign({ role: "admin", jti: crypto.randomUUID() }, env2.JWT_SECRET, { expiresIn: "12h" });
}, "signToken");
var verifyAuth = /* @__PURE__ */ __name((request, env2) => {
  const token = request.headers.get("Authorization")?.split(" ")[1];
  if (!token)
    return false;
  try {
    const decoded = import_jsonwebtoken.default.verify(token, env2.JWT_SECRET);
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var jsonResponse = /* @__PURE__ */ __name((data, status = 200, code = 0, msg = "success") => {
...
var errorResponse = /* @__PURE__ */ __name((msg, status = 400, code = 4e3, stack) => jsonResponse(null, status, code, msg, stack), "errorResponse");
var Money = {
  // 科学处理：增加微小偏移量确保 Math.round 在 0.5 时准确进位
  toCent: (yuan) => Math.round((parseFloat(yuan) + Number.EPSILON) * 100),
  toYuan: (cent) => parseFloat((cent / 100).toFixed(2)),
  calcCommission: (amountCent) => {
    const comm = Math.round(amountCent * 25e-5);
    return Math.max(500, comm);
  },
  calcTax: (amountCent) => Math.round(amountCent * 5e-4)
  // 仅卖出时调用
};
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var getFullSymbol = /* @__PURE__ */ __name((code) => {
  if (code.length !== 6)
    return code;
...
var fetchRawText = /* @__PURE__ */ __name(async (url) => {
  try {
...
var parseSinaData = /* @__PURE__ */ __name((text) => {
  ...
  return price > 0 ? { name: data[0], price } : null;
}, "parseSinaData");
var parseTencentData = /* @__PURE__ */ __name((text) => {
  const match = text.match(/="([^"]+)"/);
  if (!match)
    return null;
  const data = match[1].split("~");
  if (data.length < 4)
    return null;
  const price = parseFloat(data[3]);
  return price > 0 ? { name: data[1], price } : null;
}, "parseTencentData");
var fetchStockPrice = /* @__PURE__ */ __name(async (symbolOrCode) => {
  ...
  if (sinaRaw) {
    ...
    if (result)
      return { ...result, symbol, source: "sina" };
  }
  ...
  if (tencentRaw) {
    ...
    if (result)
      return { ...result, symbol, source: "tencent" };
  }
  return null;
}, "fetchStockPrice");

// src/trade.js
var placeOrder = /* @__PURE__ */ __name(async (env2, body) => {
  const { symbol, side, price, qty } = body;
  const marketData = await fetchStockPrice(symbol);
  if (!marketData)
    return errorResponse("\u884C\u60C5\u83B7\u53D6\u5931\u8D25", 400, 4005);
  const priceCent = Money.toCent(price);
  const tradeAmount = priceCent * qty;
  if (side === "BUY") {
    const estFee = Money.calcCommission(tradeAmount);
    const totalFreeze = tradeAmount + estFee;
    const res = await env2.DB.batch([
      env2.DB.prepare(`
                UPDATE account 
                SET frozen_balance = frozen_balance + ? 
                WHERE id = 1 AND (balance - frozen_balance) >= ?
            `).bind(totalFreeze, totalFreeze),
      env2.DB.prepare(`
                INSERT INTO orders (symbol, name, side, price, qty, status, freeze_amount) 
                VALUES (?, ?, 'BUY', ?, ?, 'PENDING', ?)
            `).bind(marketData.symbol, marketData.name, priceCent, qty, totalFreeze)
    ]);
    if (res[0].meta.changes === 0)
      return errorResponse("\u53EF\u7528\u8D44\u91D1\u4E0D\u8DB3", 400, 4001);
    return jsonResponse({ order_id: res[1].meta.last_row_id, message: "\u4E70\u5355\u5DF2\u63D0\u4EA4" });
  } else {
    const res = await env2.DB.batch([
      env2.DB.prepare(`
                UPDATE holdings 
                SET available_qty = available_qty - ? 
                WHERE symbol = ? AND available_qty >= ?
            `).bind(qty, symbol, qty),
      env2.DB.prepare(`
                INSERT INTO orders (symbol, name, side, price, qty, status) 
                VALUES (?, ?, 'SELL', ?, ?, 'PENDING')
            `).bind(marketData.symbol, marketData.name, priceCent, qty)
    ]);
    if (res[0].meta.changes === 0)
      return errorResponse("\u53EF\u7528\u6301\u4ED3\u4E0D\u8DB3(T+1\u9650\u5236)", 400, 4002);
    return jsonResponse({ order_id: res[1].meta.last_row_id, message: "\u5356\u5355\u5DF2\u63D0\u4EA4" });
  }
}, "placeOrder");
var cancelOrder = /* @__PURE__ */ __name(async (env2, orderId) => {
  const order = await env2.DB.prepare("SELECT * FROM orders WHERE id = ? AND status = 'PENDING'").bind(orderId).first();
  if (!order)
    return errorResponse("\u8BA2\u5355\u65E0\u6CD5\u64A4\u9500", 400, 4004);
  const stmts = [
    env2.DB.prepare("UPDATE orders SET status = 'CANCELLED', updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(orderId)
  ];
  if (order.side === "BUY") {
    stmts.push(env2.DB.prepare("UPDATE account SET frozen_balance = frozen_balance - ? WHERE id = 1").bind(order.freeze_amount));
  } else {
    stmts.push(env2.DB.prepare("UPDATE holdings SET available_qty = available_qty + ? WHERE symbol = ?").bind(order.qty, order.symbol));
  }
  await env2.DB.batch(stmts);
  return jsonResponse({ message: "\u64A4\u5355\u6210\u529F", order_id: orderId });
}, "cancelOrder");
var matchOrders = /* @__PURE__ */ __name(async (env2) => {
  const { results: orders } = await env2.DB.prepare("SELECT * FROM orders WHERE status = 'PENDING'").all();
  if (orders.length === 0)
    return;
  ...
  await Promise.all(symbols.map(async (s) => {
    ...
    if (data)
      marketMap[s] = Money.toCent(data.price);
  }));
  for (const order of orders) {
    const curPrice = marketMap[order.symbol];
    if (!curPrice)
      continue;
    const canFill = order.side === "BUY" ? curPrice <= order.price : curPrice >= order.price;
    if (canFill) {
      await executeTrade(env2, order, curPrice);
    }
  }
}, "matchOrders");
async function executeTrade(env2, order, exePrice) {
  ...
  const acc = await env2.DB.prepare("SELECT balance, frozen_balance FROM account WHERE id = 1").first();
  const totalAssets = acc.balance + acc.frozen_balance;
  const postPosRatio = totalAssets > 0 ? (tradeAmt / totalAssets).toFixed(4) : 0;
  const stmts = [
    env2.DB.prepare("UPDATE orders SET status = 'FILLED', filled_qty = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(order.qty, order.id)
  ];
  if (order.side === "BUY") {
    stmts.push(env2.DB.prepare("UPDATE account SET balance = balance + (? - ?), frozen_balance = frozen_balance - ? WHERE id = 1").bind(order.freeze_amount, totalSettlement, order.freeze_amount));
    const old = await env2.DB.prepare("SELECT quantity, total_cost FROM holdings WHERE symbol = ?").bind(order.symbol).first();
    if (old) {
      const nQty = old.quantity + order.qty;
      const nCost = old.total_cost + totalSettlement;
      stmts.push(env2.DB.prepare("UPDATE holdings SET quantity=?, total_cost=?, avg_cost=?, updated_at=CURRENT_TIMESTAMP WHERE symbol=?").bind(nQty, nCost, Math.round(nCost / nQty), order.symbol));
    } else {
      stmts.push(env2.DB.prepare("INSERT INTO holdings (symbol, name, quantity, available_qty, avg_cost, total_cost) VALUES (?, ?, ?, 0, ?, ?)").bind(order.symbol, order.name, order.qty, Math.round(totalSettlement / order.qty), totalSettlement));
    }
  } else {
    stmts.push(env2.DB.prepare("UPDATE account SET balance = balance + ? WHERE id = 1").bind(totalSettlement));
    stmts.push(env2.DB.prepare("UPDATE holdings SET quantity = quantity - ?, updated_at = CURRENT_TIMESTAMP WHERE symbol = ?").bind(order.qty, order.symbol));
    stmts.push(env2.DB.prepare("DELETE FROM holdings WHERE symbol = ? AND quantity <= 0").bind(order.symbol));
  }
  stmts.push(env2.DB.prepare(`
        INSERT INTO trades (order_id, symbol, name, side, price, qty, amount, commission, tax, post_pos_ratio) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(order.id, order.symbol, order.name, order.side, exePrice, order.qty, tradeAmt, fee, tax, postPosRatio));
  await env2.DB.batch(stmts);
}
__name(executeTrade, "executeTrade");
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var handleScheduled = /* @__PURE__ */ __name(async (event, env2) => {
  ...
  if (day === 0 || day === 6)
    return;
  const isTradingSession = timeValue >= 930 && timeValue <= 1130 || timeValue >= 1300 && timeValue <= 1500;
  if (isTradingSession) {
    await matchOrders(env2);
  }
  if (timeValue === 1505) {
    await dailyClosingProcess(env2);
  }
  if (timeValue === 1) {
    await env2.DB.prepare("UPDATE holdings SET available_qty = quantity").run();
    console.log("T+1 Settlement: available_qty updated.");
  }
}, "handleScheduled");
async function dailyClosingProcess(env2) {
  const { results: pendings } = await env2.DB.prepare("SELECT * FROM orders WHERE status='PENDING'").all();
  for (const order of pendings) {
    const stmts = [
      env2.DB.prepare("UPDATE orders SET status='EXPIRED', updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(order.id)
    ];
    if (order.side === "BUY") {
      stmts.push(env2.DB.prepare("UPDATE account SET frozen_balance = frozen_balance - ? WHERE id=1").bind(order.freeze_amount));
    } else {
  ...
  const account = await env2.DB.prepare("SELECT balance FROM account WHERE id=1").first();
  const { results: holdings } = await env2.DB.prepare("SELECT symbol, quantity FROM holdings WHERE quantity > 0").all();
  let marketCap = 0;
  for (const h of holdings) {
    const m = await fetchStockPrice(h.symbol);
    if (m)
      marketCap += Money.toCent(m.price) * h.quantity;
  }
  const totalAssets = account.balance + marketCap;
  const dateStr = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  await env2.DB.prepare("INSERT OR REPLACE INTO snapshots (date, total_assets) VALUES (?, ?)").bind(dateStr, totalAssets).run();
  console.log(`Closing Process Done: Assets ${totalAssets}`);
}
__name(dailyClosingProcess, "dailyClosingProcess");
...
var src_default = {
  async fetch(request, env2, ctx) {
    ...
    try {
      if (path === "/api/public/overview" && method === "GET") {
        const account = await env2.DB.prepare("SELECT * FROM account WHERE id=1").first();
        const { results: holdings } = await env2.DB.prepare("SELECT * FROM holdings").all();
        const { results: logs } = await env2.DB.prepare("SELECT * FROM trades ORDER BY id DESC LIMIT 5").all();
        const { results: snaps } = await env2.DB.prepare("SELECT * FROM snapshots ORDER BY date ASC").all();
        let marketCap = 0;
        let holdingList = [];
        let totalPnl = 0;
        for (const h of holdings) {
          const m = await fetchStockPrice(h.symbol);
          const curPrice = m ? Money.toCent(m.price) : h.avg_cost;
          const val = curPrice * h.quantity;
          marketCap += val;
          holdingList.push({
            name: h.name,
            code: h.symbol,
            cost: Money.toYuan(h.avg_cost),
            price: Money.toYuan(curPrice),
            pnl_rate: parseFloat(((curPrice - h.avg_cost) / h.avg_cost * 100).toFixed(2)),
            position_rate: 0
            // 稍后计算
          });
          totalPnl += val - h.total_cost;
        }
        const totalAssets = account.balance + marketCap;
        holdingList.forEach((h) => {
          h.position_rate = parseFloat((h.price * 100 * holdings.find((x) => x.symbol == h.code).quantity / totalAssets * 100).toFixed(2));
        });
        return jsonResponse({
          assets: {
            total: Money.toYuan(totalAssets),
            market_cap: Money.toYuan(marketCap),
            balance: Money.toYuan(account.balance),
            pnl_holding: Money.toYuan(totalPnl),
            return_total_pct: parseFloat(((totalAssets - account.initial_capital) / account.initial_capital * 100).toFixed(2))
          },
          charts: {
            asset: snaps.map((s) => ({ date: s.date, value: Money.toYuan(s.total_assets) }))
          },
          logs: logs.map((l) => ({
            text: `${l.side} ${l.name} ${l.qty}\u80A1 @ ${Money.toYuan(l.price)}`,
            time: l.trade_time
          })),
      ...
      if (path === "/api/auth/login" && method === "POST") {
        ...
        if (await verifyPassword(body.password, body.password, env2)) {
        }
        const isValid = await verifyPassword(body.password, null, env2);
        if (isValid) {
          return jsonResponse({ token: signToken(env2) });
        }
        return errorResponse("Invalid Password", 401);
      }
      if (path.startsWith("/api/admin/")) {
        const isAuth = await verifyAuth(request, env2);
        if (!isAuth)
          return errorResponse("Unauthorized", 401);
        if (path === "/api/admin/dashboard" && method === "GET") {
          const acc = await env2.DB.prepare("SELECT * FROM account WHERE id=1").first();
          return jsonResponse({
            balance: Money.toYuan(acc.balance),
            frozen: Money.toYuan(acc.frozen_balance),
            available: Money.toYuan(acc.balance - acc.frozen_balance)
          });
        }
        if (path === "/api/admin/trade" && method === "POST") {
          const body = await request.json();
          const res = await placeOrder(env2, body);
          ctx.waitUntil(matchOrders(env2));
          return res;
        }
        if (path === "/api/admin/cancel" && method === "POST") {
          const body = await request.json();
          return await cancelOrder(env2, body.order_id);
        }
        if (path === "/api/admin/orders" && method === "GET") {
          const { results } = await env2.DB.prepare("SELECT * FROM orders ORDER BY id DESC LIMIT 50").all();
          return jsonResponse(results.map((o) => ({
            ...o,
            price: Money.toYuan(o.price)
          })));
        }
        if (path === "/api/admin/holdings" && method === "GET") {
          const { results } = await env2.DB.prepare("SELECT * FROM holdings").all();
          return jsonResponse(results.map((h) => ({
            ...h,
      ...
      console.dir(e);
      return errorResponse(e.message, 500, null, e.stack);
    }
  },
  // 定时任务入口
