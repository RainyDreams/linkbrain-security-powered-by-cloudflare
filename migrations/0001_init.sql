...
CREATE INDEX IF NOT EXISTS idx_bank_transfers_date ON bank_transfers(cst_date, status);
...
CREATE INDEX IF NOT EXISTS idx_audit_tech_time ON audit_technical(created_at_cst DESC);
CREATE INDEX IF NOT EXISTS idx_audit_tech_scope ON audit_technical(scope, status);
CREATE INDEX IF NOT EXISTS idx_audit_tech_order ON audit_technical(order_id, symbol, request_id);
...
CREATE INDEX IF NOT EXISTS idx_audit_fin_time ON audit_financial(created_at_cst DESC);
CREATE INDEX IF NOT EXISTS idx_audit_fin_scope ON audit_financial(scope, status);
CREATE INDEX IF NOT EXISTS idx_audit_fin_order ON audit_financial(order_id, symbol, request_id);
CREATE INDEX IF NOT EXISTS idx_audit_fin_amount ON audit_financial(amount, fee, tax);
...
INSERT OR IGNORE INTO account (id, balance, frozen_balance, initial_capital)
VALUES (1, 100000000, 0, 100000000);

INSERT OR IGNORE INTO snapshots (date, total_assets, day_pnl, total_return_rate, max_drawdown)
